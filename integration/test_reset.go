			...
			go func(i int) {
				...
				checkValue := func(mfs map[string]*dto.MetricFamily, wantValue int) {
					for _, mf := range mfs {
						...
						if wantValue == 0 {
							log.Fatalf("metric %s exists while it should not; %+v", metricName, mf)
						}
						for _, m := range mf.Metric {
							if len(m.Label) != 1 {
								log.Fatalf("expected metric %s to have 1 label, got %+v", metricName, m)
							}
							if m.Label[0].GetValue() != labelValue {
								continue
							}
							if m.GetGauge().GetValue() != float64(wantValue) {
								log.Fatalf("expected metric %s to have value of %d, got %+v", metricName, wantValue, m)
							}
							return
						}
						log.Fatalf("metric %s does not have label %s while it should; %+v", metricName, labelValue, mf)
					}
					if wantValue != 0 {
						log.Fatalf("metric %s not found in %+v", metricName, mfs)
					}
				}
				for start := time.Now(); time.Since(start) < *testDuration; {
					// Call the URL that sets a label value and confirm that it
					// exists in the returned metrics.
					value := rand.Intn(9000)
					mu.RLock()
					...
					// attempt to change or change the gauge at the same time.
					if rand.Intn(100) < 5 {
						mu.Lock()
						tr.mustGet(resetURL)
						metrics := tr.mustGetMetrics(tr.ctx)
