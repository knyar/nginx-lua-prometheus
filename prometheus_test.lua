...
function TestPrometheus:testCollect()
  ...
  payload = self.p:collect()

  assert(string.match(payload, "# HELP metric1 Metric 1"))
  assert(string.match(payload, "# TYPE metric1 counter"))
  assert(string.match(payload, "metric1 5"))

  assert(string.match(payload, "# TYPE metric2 counter"))
  assert(string.match(payload, 'metric2{f2="v2",f1="v1"} 4'))

  assert(string.match(payload, "# TYPE gauge1 gauge"))
  assert(string.match(payload, 'gauge1 3'))

  assert(string.match(payload, "# TYPE gauge2 gauge"))
  assert(string.match(payload, 'gauge2{f2="v2",f1="v1"} 5'))

  assert(string.match(payload, "# TYPE b1 histogram"))
  assert(string.match(payload, "# HELP b1 Bytes"))
  assert(string.match(payload, 'b1_bucket{var="ok",le="0100.0"} 2'))
  assert(string.match(payload, 'b1_sum{var="ok"} 5250'))

  assert(string.match(payload, 'l2_bucket{var="ok",site="site2",le="04.000"} 2'))
  assert(string.match(payload, 'l2_bucket{var="ok",site="site2",le="%+Inf"} 4'))

  -- check that type comment exists and is before any samples for the metric.
  assert(string.find(payload, '# TYPE l1 histogram\nl1'))
  luaunit.assertEquals(ngx.logs, nil)
end
...
function TestPrometheus:testCollectWithPrefix()
  ...
  payload = p:collect()
  assert(string.match(payload, "# HELP test_pref_metric1 Metric 1"))
  assert(string.match(payload, "# TYPE test_pref_metric1 counter"))
  assert(string.match(payload, "test_pref_metric1 5"))
  assert(string.match(payload, "# HELP test_pref_gauge1 Gauge 1"))
  assert(string.match(payload, "# TYPE test_pref_gauge1 gauge"))
  assert(string.match(payload, "test_pref_gauge1 3"))
  assert(string.match(payload, "# TYPE test_pref_b1 histogram"))
  assert(string.match(payload, "# HELP test_pref_b1 Bytes"))
  assert(string.match(payload, "# HELP test_pref_metric1 Metric 1"))
  assert(string.match(payload, 'test_pref_b1_bucket{var="ok",le="0100.0"} 2'))
  assert(string.match(payload, 'test_pref_b1_sum{var="ok"} 5250'))
end
