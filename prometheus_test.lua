...
function TestPrometheus:testCollectWithPrefix()
  ...
  assert(find_idx(payload, "# HELP test_pref_metric1 Metric 1") ~= nil)
  assert(find_idx(payload, "# TYPE test_pref_metric1 counter") ~= nil)
  assert(find_idx(payload, "test_pref_metric1 5") ~= nil)

  assert(find_idx(payload, "# HELP test_pref_gauge1 Gauge 1") ~= nil)
  assert(find_idx(payload, "# TYPE test_pref_gauge1 gauge") ~= nil)
  assert(find_idx(payload, "test_pref_gauge1 3") ~= nil)

  assert(find_idx(payload, "# TYPE test_pref_b1 histogram") ~= nil)
  assert(find_idx(payload, "# HELP test_pref_b1 Bytes") ~= nil)
  assert(find_idx(payload, 'test_pref_b1_bucket{var="ok",le="0100.0"} 2') ~= nil)
  assert(find_idx(payload, 'test_pref_b1_sum{var="ok"} 5250') ~= nil)
end
